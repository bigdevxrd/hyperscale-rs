...
- **Invariant IDs** (`INV-<AREA>-<n>`) are stable references. They appear inline in each doc where the property arises; the precise statements live once, in [08-invariants.md](08-invariants.md). Cite them rather than restating properties. The `INV-VM-*` family is the exception: the execution engine documents itself, and its register is [vm/docs/08-invariants.md](../vm/docs/08-invariants.md).
