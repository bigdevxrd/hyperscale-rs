...
Transaction execution itself sits behind a batch-shaped **executor seam** (`Executor` in `crates/engine`): a wave's transactions split into per-engine sub-batches by body variant — a transaction is wholly Radix or wholly VM, typed on the wire, never flagged — and each engine returns one receipt per transaction, merged in canonical order. The Radix implementation is the per-transaction engine loop; the VM implementation (`crates/engine-vm`) hands its whole sub-batch to the kernel's deterministic-parallel executor, whose receipts are schedule-invariant: group scheduling (serial, parallel, adversarial timing) cannot change any receipt byte, so the scheduling mode is a performance knob, never a consensus input. VM guests run on the pinned wasmtime engine natively and on the reference interpreter on wasm32, with byte-identical receipts and fuel across both runtimes held by the VM repository's differential suite — determinism at the seam is verified by two independent implementations, not assumed.
