...
Transaction execution itself is batch-shaped (`Executor` in `crates/engine`): a wave's transactions go to the kernel's deterministic-parallel executor whole, and it returns one receipt per transaction in input order. Those receipts are schedule-invariant — group scheduling (serial, parallel, adversarial timing) cannot change any receipt byte, so the scheduling mode is a performance knob and never a consensus input. Guests run on the pinned wasmtime engine natively and on the reference interpreter on wasm32, with byte-identical receipts and fuel across both runtimes held by the VM repository's differential suite — determinism at the execution boundary is verified by two independent implementations, not assumed.
