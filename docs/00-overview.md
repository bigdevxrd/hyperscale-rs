...
Liveness is protected by construction rather than by retry. The mempool's ready set enforces that no two in-flight transactions share any declared key, so deadlocks between local transactions cannot form. And every wave carries a deadline derived from BFT-attested time, after which it deterministically all-aborts — which is also what breaks a true cross-shard cycle, since every replica derives the same deadline from attested content alone. Every transaction terminates; no lock is held forever; no two shards ever disagree on the terminal outcome.
